  ...
  return (
    <section style={{padding:'120px 0 40px'}} data-screen-label="Equipo">
      <style>{`
        ...
        .tm-photo .ph { width: 100%; height: 100%; }
        .tm-name {
      ...
      <div className="container">
        <SectionHead
          eyebrow={{num: "05 — Quiénes somos", text: "Brokers en planta"}}
          title='Un equipo <em>pequeño</em>,<br/>una libreta <em>grande</em>.'
          kicker="Seis brokers, una arquitecta restauradora y un abogado fiduciario. No subcontratamos atención: la persona que te contesta es la que cierra contigo."
        />
        <div className="team-grid">
          {TEAM.map((t, i) => (
            <div className="tm" key={t.name}>
              <div className="tm-photo">
                <image-slot id={`slot-team-${t.name.split(' ')[0].toLowerCase()}`} placeholder={`Retrato · ${t.name}`} style={{width:'100%', height:'100%', display:'block'}}></image-slot>
              </div>
              <div>
        ...
        <div className="jr-grid">
          {[
            ...
            <article className="jr" key={i}>
              <div className="jr-img"><image-slot id={`slot-journal-${i}`} placeholder={`Diario · ${j.cat}`} style={{width:'100%', height:'100%', display:'block'}}></image-slot></div>
